...
export class convertNumbersToWords {
  ...
  protected convertInternal(
    ...
    if (locale.config.exactWordsMapping) {
      ...
      if (exactMatch) {
        return [
          Array.isArray(exactMatch.value)
            ? exactMatch.value[+trailing]
            : exactMatch.value,
        ];
      }
    }
    ...
    if (
      quotient === 1 &&
      locale.config?.ignoreOneForWords?.includes(
        Array.isArray(matchValue) ? matchValue[0] : matchValue,
      )
    ) {
      words.push(Array.isArray(matchValue) ? matchValue[1] : matchValue);
    } else if (
      quotient === 1 &&
      locale.config?.specialNumberCase?.some(elem => {
        return Math.floor(number / elem.number) === 1;
      })
    ) {
      const match = locale.config.specialNumberCase.find(elem => {
        return Math.floor(number / elem.number) === 1;
      }) as NumberWordMap;
      const matchValue = match.value;
      words.push(Array.isArray(matchValue) ? matchValue[0] : matchValue);
    } else {
      words.push(
        ...this.convertInternal(quotient, false),
        Array.isArray(matchValue) ? matchValue[0] : matchValue,
      );
    }
    ...
    if (remainder > 0) {
      if (locale.config?.splitWord?.length) {
        if (
          !locale.config?.noSplitWordAfter?.find(word => word === match.value)
        ) {
          words.push(locale.config.splitWord);
        }
      }
